...
%% Functions
  ...
  function show(h,o)
    ...
    if ~isempty(pNum)
      ...
      % hdi
      t1 = text(f.SP(1).H,hdi(1),-.5*hdiHig,roundDec(hdi(1),2),...
        'horizontalalignment','left','fontsize',12);
      t2 = text(f.SP(1).H,hdi(2),-.5*hdiHig,roundDec(hdi(2),2),...
      ...
      % summary
      modeText = text(f.SP(1).H,.8 ,.9, ['Mode = ' roundDec(post.mode,2)],'units','normalized',...
        'horizontalalignment','left','fontsize',14,'backgroundcolor',[ 1 1 1]);
      meanText = text(f.SP(1).H,.8 ,.85, ['Mean = ' roundDec(post.mu,2)],'units','normalized',...
        'horizontalalignment','left','fontsize',14,'backgroundcolor',[ 1 1 1]);
      sigmaText = text(f.SP(1).H,.8 ,.8, ['SD = ' roundDec(post.sig,2)],'units','normalized',...
        'horizontalalignment','left','fontsize',14,'backgroundcolor',[ 1 1 1]);
