...
if isnan(prev.params)
  for p = 1:model.n.params
    prop.params(:,p) = random(...
      model.bayes.priors{p,1},...
      model.bayes.priors{p,2},...
      model.bayes.priors{p,3},[model.settings.n.chains,1]);
  end
  
  prop.mode = zeros(model.settings.n.chains,1);
  
  % scale the variance
  % % %   for ch = 1:model.settings.n.chains
  % % %     for p = 1:model.n.params
  % % %       [v1,v2] = mlb_convertPrior(model,prop.params(ch,:),p);
  % % %       range = icdf(model.bayes.priors{p,1},...
  % % %         [.25,.75],...
  % % %         v1,...
  % % %         v2);
  % % %       model.settings.sampling.varianceScaled(ch,p) = model.settings.sampling.variance * diff(range);
  % % %     end
  % % %   end
else
