...
upNDX = 1;
for ndx = 1:length(progdata)
  if input{ndx} < 1 && input{ndx} > 0 
    upNDX = ndx;
  end
end
if progdata(upNDX).fractiondone > 0
    runtime = etime(progdata(upNDX).clock, progdata(upNDX).starttime);
    timeleft = runtime / progdata(upNDX).fractiondone - runtime;
    timeleftstr = sec2timestr(timeleft);
    titlebarstr = sprintf('%2d%%    %s remaining', ...
        floor(100*progdata(upNDX).fractiondone), timeleftstr);
else
    titlebarstr = ' 0%';
end
